...
resource "aws_iam_openid_connect_provider" "main" {
  ...
  url             = var.aws_eks_cluster_name.identity[0].oidc[0].issuer
}
  ...
  "Statement": [
    {
      "Effect": "Allow",
      "Principal": {
        "Federated": "arn:aws:iam::${data.aws_caller_identity.current.account_id}:oidc-provider/${replace(var.aws_eks_cluster_name.identity[0].oidc[0].issuer, "https://", "")}"
      },
      "Action": "sts:AssumeRoleWithWebIdentity",
      "Condition": {
        "StringEquals": {
          "${replace(var.aws_eks_cluster_name.identity[0].oidc[0].issuer, "https://", "")}:sub": "system:serviceaccount:kube-system:alb-ingress-controller"
        }
      }
    }
  ]
...
data "external" "thumbprint" {
  program    = ["${path.module}/oidc.sh", var.region]
  depends_on = [var.aws_eks_cluster_eks]
}
